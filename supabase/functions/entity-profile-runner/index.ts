// Auto-discovers new person entities (≥3 episodes across ≥2 podcasts) and refreshes
// existing person profiles when new episodes mention them.
...
const KIND_BUDGET = "entity_profile_person";
...
async function generate(slug: string, displayName: string | null, force: boolean): Promise<{ ok: boolean; cost?: number; error?: string }> {
  try {
    const r = await fetch(`${SUPABASE_URL}/functions/v1/entity-profile-generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${SERVICE_ROLE}` },
      body: JSON.stringify({ kind: "person", slug, force }),
    });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ---- REFRESH PASS (cheaper, keep existing pages fresh) ----
    const { data: refreshCands, error: refErr } = await sb.rpc("select_person_refresh_candidates", { _limit: maxRefresh });
    ...
    // ---- DISCOVERY PASS (new persons) ----
    const { data: newCands, error: newErr } = await sb.rpc("select_person_candidates", {
