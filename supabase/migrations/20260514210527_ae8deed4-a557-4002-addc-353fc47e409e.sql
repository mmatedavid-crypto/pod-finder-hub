CREATE OR REPLACE FUNCTION public.search_episodes_hybrid(q text, q_embedding vector DEFAULT NULL::vector, limit_n integer DEFAULT 50, lang text DEFAULT 'en'::text, required_terms text[] DEFAULT NULL::text[], entity_terms text[] DEFAULT NULL::text[], alpha_lex numeric DEFAULT 0.5)
      ...
      AND (
        ...
        OR NOT EXISTS (
          ...
          WHERE length(btrim(rt.term)) >= 3
            AND lower(coalesce(e.search_text,'')) !~* ('\m' || lower(btrim(rt.term)) || '\M')
        )
      )
    ORDER BY r DESC
    LIMIT 200
