DROP FUNCTION IF EXISTS public.token_idf(text[]);
CREATE FUNCTION public.token_idf(p_tokens text[])
...
-- Invalidate stale zero-cached entries computed under wrong dictionary
DELETE FROM public.token_df_cache WHERE df = 0;